	...
	void SUCommandReceiver(){
		...
		// Process to use the declaration
	     ...
	     try {
	 		...
	 		String[] sucommand = new String[] {
					"su",
					"-c" + sucommand_receiver };
	 		// 수정한 데이터를 실행한다
	 		// Modify data, run
	    	 processcommand = sucommand_runtime.exec(sucommand); 
	    	// 버퍼를 사용한다
	    	// Use Buffer
	    	 BufferedReader br = new BufferedReader(new InputStreamReader(processcommand.getInputStream()));
	    	 // 문구가 오류를 일으키거나 버퍼가 문제 있을 때 디버깅으로 알려준다.
	    	 // Tells the error terms can cause debugging problems when the buffer is.
             String line ; 
	    	 ...
	    	 Log.e("SUAdapter", "SOME ERROR(S) Detected in SUAdapter in SUAdapter.java!");
	     }
	}
}
